...
- **Grade 1**： 追加・代替処置を要するが、生命を脅かさず、臓器の一部または全摘出を伴わない。長期的な後遺症を残さない。
- **Grade 2**： 主要な追加・代替処置を要する。短期または長期的な副作用を残す可能性がある。
- **Grade 3**： 直ちに生命を脅かす事態だが、臓器の一部または全摘出は要さないもの。
- **Grade 4**： 直ちに生命を脅かす事態。4A: 臓器の一部または全摘出、4B: 手術完了不能。
...
if 'init_done' not in st.session_state:
    ...
    defaults = {
        ...
        "vital_detail": "N/A", "bladder_tumor_tx": "N/A", "op_date": None, "op_admission_date": None, "op_discharge_date": None,
        ...
        "trg_grade": "未選択", "no_op_reason": "未選択", "cd_grade": "未選択", "cd_detail": "N/A",
        "adj_plan": "選択してください", "adj_other_detail": ""
    }
...
with tab1:
    ...
    with bc1:
        wbc = st.number_input("WBC (/μL)*", value=0, step=1, max_value=100000)
        hb = st.number_input("Hb (g/dL)*", value=0.0, step=0.1, max_value=25.0)
        plt = st.number_input("PLT (x10^4/μL)*", value=0, step=1, max_value=200)
        ast = st.number_input("AST (U/L)*", value=0, step=1, max_value=5000)
        alt = st.number_input("ALT (U/L)*", value=0, step=1, max_value=5000)
    with bc2:
        ldh = st.number_input("LDH (U/L)*", value=0, step=1, max_value=10000)
        alb = st.number_input("Alb (g/dL)*", value=0.0, step=0.1, max_value=10.0)
        cre = st.number_input("Cre (mg/dL)*", value=0.00, step=0.01, max_value=20.0)
        egfr = st.number_input("eGFR (mL/min/1.73m²)*", value=0.0, step=0.1, max_value=200.0)
        crp = st.number_input("CRP (mg/dL)*", value=0.00, step=0.01, max_value=50.0)
...
with tab2:
    ...
    if op_performed == "実施した":
        ...
        with oc2:
            ...
            if st.session_state.eau_grade not in ["選択してください", "Grade 0"]: st.session_state.eau_detail = st.text_area("詳細*")
...
with tab3:
    if op_performed == "実施した":
        ...
        with pc1:
            ...
            st.session_state.p_morphology = st.selectbox("形態*", ["選択してください", "乳頭状", "非乳頭状", "結節状", "浸潤状", "平坦状", "その他"], index=0)
        ...
        with pc2:
            ...
            st.session_state.trg_grade = st.radio("病理学的治療効果（TRG分類）*", ["TRG 1", "TRG 2", "TRG 3"], index=None, help=HELP_TRG)
...
with tab4:
    ...
    with sc1:
        if op_performed == "実施した":
            ...
            if st.session_state.cd_grade not in ["選択してください", "Grade 0"]: st.session_state.cd_detail = st.text_area("CD詳細*")
        else: st.session_state.cd_grade = "N/A"
    with sc2:
        st.session_state.adj_plan = st.selectbox("今後の治療予定（または実施中）*", ["選択してください", "無治療", "EVP継続", "ペムブロ単剤", "ニボルマブ単剤", "プラチナ製剤", "その他"], index=0)
        if st.session_state.adj_plan == "その他": st.session_state.adj_other_detail = st.text_area("治療内容詳細*", height=200)
        adj_date = st.date_input("次回治療開始/実施日*", value=None)
    ...
    if st.button("🚀 事務局へ確定送信", type="primary", use_container_width=True):
        # バリデーション（ID・施設・日付）
        if not st.session_state.patient_id or st.session_state.facility_name == "選択してください": st.error("施設名と識別コードを入力してください")
        elif st.session_state.status_alive is None: st.error("生存状況を選択してください")
        elif op_performed == "実施した" and st.session_state.op_date and last_evp_date and st.session_state.op_date <= last_evp_date: st.error("【日付エラー】手術日は最終EVP投与日より後の日付にしてください")
        else:
            # 送信レポート全文構成（血液検査・分画を全て含む）
            rep = f"""施設: {st.session_state.facility_name} / ID: {st.session_state.patient_id}
【術前血液検査】
WBC: {wbc}, Hb: {hb}, PLT: {plt}, AST: {ast}, ALT: {alt}, LDH: {ldh}, Alb: {alb}, Cre: {cre}, eGFR: {egfr}, CRP: {crp}
【分画】
Neutro: {neutro}%, Lympho: {lympho}%, Mono: {mono}%, Eosino: {eosino}%, Baso: {baso}%
【手術】
実施: {op_performed}, 日付: {st.session_state.op_date}, EAU: {st.session_state.eau_grade}
【病理】
ypT: {st.session_state.ypt}, ypN: {st.session_state.ypn}, TRG: {st.session_state.trg_grade}
【術後30日】
生存: {st.session_state.status_alive}, CD分類: {st.session_state.cd_grade}, 予定: {st.session_state.adj_plan}"""
