...
with tab3:
    if st.session_state.op_performed == "実施した":
        ...
        # 指示：評価不能時の理由記載欄を動的に表示
        if any(x == "評価不能" for x in [st.session_state.p_histology, st.session_state.p_morphology, st.session_state.ypt, st.session_state.ypn, st.session_state.p_lvi, st.session_state.r0_status, st.session_state.trg_grade]):
            st.markdown("---")
            st.session_state.p_eval_failed_reason = st.text_area("病理評価不能の理由*", placeholder="例：術前治療の効果で残存がん細胞が消失しているため、焼灼変性のため、など詳細を記載してください。")
...
with tab4:
    ...
    if not st.session_state.needs_confirm:
        if st.button("🚀 事務局へ確定送信", type="primary", use_container_width=True):
            ...
            # 指示：病理評価不能を選んだ際の理由必須化
            if st.session_state.op_performed == "実施した":
                if any(x == "評価不能" for x in [st.session_state.p_histology, st.session_state.p_morphology, st.session_state.ypt, st.session_state.ypn, st.session_state.p_lvi, st.session_state.r0_status, st.session_state.trg_grade]):
                    if not st.session_state.p_eval_failed_reason:
                        h_errors.append("・病理評価不能の理由")

            if h_errors:
                st.error("【入力エラー】以下の必須項目を正しく入力してください：\n" + "\n".join(h_errors))
            else:
                # 2. 警告(Soft)チェック：緩和対象
                s_errors = []
    ...
    if st.session_state.needs_confirm:
        st.warning(f"【確認】未入力・未選択の項目があります： " + ", ".join(st.session_state.pending_s_errors) + "\n\n現時点で入力が困難な場合は、このまま送信可能です。送信しますか？")
...
術式: {st.session_state.op_type} / アプローチ: {st.session_state.approach} / 完遂: {st.session_state.op_completed} (理由: {st.session_state.op_incomplete_detail}) / EAU: {st.session_state.eau_grade}
...
※病理評価不能の理由: {st.session_state.p_eval_failed_reason if st.session_state.p_eval_failed_reason else "なし"}
...
"""
        if send_email(rep, st.session_state.patient_id, st.session_state.facility_name):
            st.success("送信完了しました。評価不能の理由を含め、全項目が受理されました。")
            st.balloons()
