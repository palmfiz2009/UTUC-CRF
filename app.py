...
st.markdown("""
    ...
    h1 { 
        ...
        margin-bottom: 80px !important; /* 80pxの余白 */
        font-weight: 800; 
        height: 40px;
    }
    .top-info-bar {
...
- **Grade 0**： 介入や手術アプローチの変更を要さず、予定された手順からの逸脱がないもの。
- **Grade 1**： 追加処置を要するが生命を脅かさず、後遺症を残さない。
- **Grade 2**： 主要な追加・代替処置を要するが生命を脅かさない。
- **Grade 3**： 手順に加え主要な追加処置を要し、かつ事象が生命を脅かすもの。
- **Grade 4**： 短期または長期的な重大な結果をもたらす（4A: 臓器摘出、4B: 手術完了不能）。
- **Grade 5**： 5A: 部位間違い等、5B: 術中死亡。"""

HELP_TRG = """**TRG (Tumor Regression Grade) 分類**
- **TRG 1：Complete Response**: 生存がん細胞なし。広範な線維化。
- **TRG 2：Strong Response**: 線維化優位。生存がん細胞50%未満。
- **TRG 3：Weak and No Response**: 生存がん細胞50%以上、または変化なし。"""

HELP_CD = """**Clavien-Dindo 分類 (術後30日以内)**
Gradingの原則：
- **Grade I**：薬物・外科的治療等を要さない。
- **Grade II**：制吐剤・解熱剤等以外の薬物療法（輸血、TPN含む）を要する。
- **Grade III**：外科的・内視鏡的治療等を要する（IIIa: 局麻、IIIb: 全麻）。
- **Grade IV**：生命を脅かす合併症（IVa: 単一臓器不全、IVb: 多臓器不全）。
- **Grade V**：患者の死亡。"""

# --- セッション状態初期化 (全ての脱落項目を再定義) ---
...
with tab3:
    if st.session_state.op_performed == "実施した":
        ...
        with pc2:
            ...
            st.session_state.trg_grade = st.radio("病理学的治療効果（TRG分類）*", trg_opts, index=None, help=HELP_TRG)

        if "評価不能" in [st.session_state.p_histology, st.session_state.ypt, st.session_state.ypn, st.session_state.trg_grade]:
...
with tab4:
    ...
    if st.button("🚀 事務局へ確定送信", type="primary", use_container_width=True):
        ...
        if h_errors:
            ...
            rep = f"【JUOG 周術期報告】\n施設: {d.facility_name}\nID: {d.patient_id}\n生存: {d.status_alive} (最終確認: {d.final_visit_date_30 if d.status_alive=='生存' else d.death_date_30})\n主要採血: WBC:{f_num(wbc)}, Hb:{f_num(hb)}, Cre:{f_num(cre)}"
